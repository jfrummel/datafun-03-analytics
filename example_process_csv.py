...
def analyze_ladder_score(file_path: pathlib.Path) -> dict:
    """Analyze the Goals column to calculate min, max, mean, and stdev."""
    try:
        ...
        with file_path.open('r') as file:
            ...
            for row in dict_reader:
                try:
                    score = float(row["Goals"])  # Extract and convert to float
...
def process_csv_file():
    ...
    input_file = pathlib.Path(fetched_folder_name, "soccer.csv")
    output_file = pathlib.Path(processed_folder_name, "soccer_goals_stats.txt")
    
    stats = analyze_ladder_score(input_file)
    output_file.parent.mkdir(parents=True, exist_ok=True)
    
    with output_file.open('w') as file:
        file.write("Goal Statistics:\n")
